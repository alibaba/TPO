...
class DPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        if reference_free:
            ref_logratios = 0
        print(f"chose_score type: {type(chose_score)}")
        print(chose_score)
        print(f"rej_score type: {type(rej_score)}")
        print(rej_score)
        tao = chose_score/(chose_score+rej_score)
        tao2 = rej_score / chose_score
        logits = pi_logratios - ref_logratios
        logits2 = ref_logratios - pi_logratios
        if self.loss_type == "sigmoid":
            losses = -F.logsigmoid(self.beta * logits)
            # losses = -tao2 * F.logsigmoid(self.beta * logits)
            # losses = -(tao * F.logsigmoid(self.beta * logits) + (1-tao) * F.logsigmoid(self.beta * logits2))
    ...
    def _get_token_batch_logps(
        self,
        logits: torch.FloatTensor,
        rejected_logits_own: torch.FloatTensor,
        labels: torch.LongTensor,
        average_log_prob: bool = False,
    ) -> torch.FloatTensor:
        """Compute the log probabilities of the given labels under the given logits.
        ...
        # 计算在 labels 对应位置的 logits1 和 logits2 的差
        # 1. 使用 labels 作为索引，提取对应的 logits1 和 logits2，去掉最后一个维度以获得形状 (batch_size, sequence_length)
        logits1_selected = logits.gather(2, labels.unsqueeze(-1)).squeeze(-1)
        logits2_selected = rejected_logits_own.gather(2, labels.unsqueeze(-1)).squeeze(-1)

        # 2. 计算它们的差并生成 score
        score = logits1_selected - logits2_selected  # Shape: (batch_size, sequence_length)
        # print(f"logits1_selected:{logits1_selected}")
        # print(f"logits2_selected:{logits2_selected}")
        # print("##############")
        # print(f"score:{score}")
        # print(torch.count_nonzero(score))
        # print(loss_mask.sum(-1))
        epsilon = 1e-8 
        avg_score = (score* loss_mask).sum(-1) / (loss_mask.sum(-1) + epsilon)
        # print(f"avg_score:{avg_score}")
        # min_value = 0.5
        # max_value = 2
        # # epsilon = 1e-8  # 小的值，避免除以零
        # # 将 score 进行偏移以避免除以零
        # # score = 1.0 / (score + epsilon) + 1
        ...
        per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2) + torch.log(score)
        # 按理说我应该是要在这里加上一个socre的，然后你的dpo_loss正常不变就行。
        if average_log_prob:
            return (per_token_logps * loss_mask).sum(-1) / loss_mask.sum(-1),avg_score
        else:
            return (per_token_logps * loss_mask).sum(-1),avg_score

    def _get_token_batch_logps_reciprocal(
        self,
        logits: torch.FloatTensor,
        rejected_logits_own: torch.FloatTensor,
        labels: torch.LongTensor,
        ...
        # score = torch.clamp(score, min=min_value, max=max_value)
        # score[score < 0] = 2
        score=(1-torch.sigmoid(score))+0.5
        # score=-torch.log(torch.sigmoid(score))
    ...
    def concatenated_forward(
        ...
        chosen_logps,chose_score = self._get_token_batch_logps(
            chosen_logits,
            chosen_logits_noisy,
            new_chosen_labels,
        ...
        #     chosen_logits, # _get_token_batch_logps_reciprocal o
        #     new_chosen_labels,
        #     average_log_prob=False, # _get_token_batch_logps w
        # )
        ...
        rejected_logps,rej_score = self._get_token_batch_logps_reciprocal(
            rejected_logits,
            rejected_logits_noisy,
            new_rejected_labels,
            average_log_prob=False,
        )

        # rejected_logps = self._get_token_batch_logps_reciprocal(
        #     rejected_logits,
        #     new_rejected_labels,
        #     average_log_prob=False,
        # )

        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits,chose_score,rej_score)
